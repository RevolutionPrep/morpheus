...
describe Morpheus::Finders, "#find" do
  ...
  context "when the argument is a single integer" do
    ...
    context "when this record does not exist on the external service" do
      ...
      before(:each) do
        Morpheus::Configuration.hydra.stub(:get, "#{Morpheus::Configuration.host}/dogs/1").and_return(build_polaris_response(404, nil))
      end
  ...
  context "when the argument is a single array" do
    ...
    context "when there is a requested record that does not exist on the external service" do
      ...
      before(:each) do
        Morpheus::Configuration.hydra.stub(:get, "#{Morpheus::Configuration.host}/dogs?ids%5B%5D=1&ids%5B%5D=2&ids%5B%5D=3").and_return(build_polaris_response(404, nil))
      end
